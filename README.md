...
运行 `DSHLauncher-Setup-0.1.1-x64.exe`。
...
已经安装过 DSH Launcher 时，可以运行 `DSHLauncher-Update-0.1.1-x64.exe`。该轻量更新包只替换 Launcher 主程序，不安装 Node.js、不修改 DSH 程序包，也不更改 DSH_HOME 和现有配置。
...
artifacts\installer\DSHLauncher-Setup-0.1.1-x64.exe
artifacts\updater\DSHLauncher-Update-0.1.1-x64.exe
```
